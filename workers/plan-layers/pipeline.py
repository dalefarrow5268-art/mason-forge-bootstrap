...
def run(source, page_number, profile, output):
    ...
    if profile.get('adapter')=='auto-revit-candidates':
        # Select drawing/text views by evidence density, never a fixed Bradenton view ID.
        wall_votes=defaultdict(int); room_votes=defaultdict(int)
        for k,rec in groups.items():
            for region in rec['regions']:
                if rec['maxwidth']>=1.2 or rec['gray']: wall_votes[region]+=1
