...
export function normalizeScan(r){if(!r||!Array.isArray(r.findings))return r;return {...r,findings:r.findings.map(f=>f&&typeof f==='object'?{...f,content:structuredText(f.content),location:structuredText(f.location)}:f)};}
...
export function nativeCapturePrompt(){return ` A nativeCapture record from the same preserved vector PDF is included in Context. Use its coordinate-preserving textBlocks and words as primary evidence for fine print, schedules, notes, dimensions, symbols with text, and rotated labels that are visually small in the rendered page. Cross-check that extracted text against the supplied PDF; do not treat extraction as a verified scale or invent content absent from both sources.`;}
export const nativeCaptureGateReady=(items,expected)=>Number(expected)>0&&items.length===Number(expected)&&items.every(item=>item.asset_role==='NATIVE_PAGE'&&item.capture_path);
...
export async function queueHoldingScan(env){
 ...
 for(const p of waiting){
  ...
  // Native capture v2 is the fast, deterministic first scan. Its verified package
  // already contains one coordinate-preserving capture beside every PDF page.
  // Detailed model interpretation continues below, but does not hold Phase One.
  const items=(await env.DB.prepare('SELECT asset_role,capture_path FROM holding_scan_items WHERE source_file_id=? ORDER BY entry_index').bind(p.source_file_id).all()).results||[];
  const captured=nativeCaptureGateReady(items,expected);
  await env.DB.prepare("UPDATE holding_preparations SET status=?,updated_at=? WHERE source_file_id=? AND status='READY'").bind(captured?'SCANNED':'SCANNING',now(),p.source_file_id).run();
 }
 // Reconcile packages created before capture and interpretation were separated.
 // This changes only the preparation gate; existing semantic review rows and
 // their Brain records remain untouched and continue below.
 await env.DB.prepare(`UPDATE holding_preparations SET status='SCANNED',error=NULL,updated_at=?
  WHERE status='SCANNING' AND COALESCE(scan_units_total,units_done)>0
  AND COALESCE(scan_units_total,units_done)=(SELECT COUNT(*) FROM holding_scan_items i WHERE i.source_file_id=holding_preparations.source_file_id)
  AND NOT EXISTS(SELECT 1 FROM holding_scan_items i WHERE i.source_file_id=holding_preparations.source_file_id AND (i.asset_role!='NATIVE_PAGE' OR i.capture_path IS NULL))`).bind(now()).run();
 const tasks=(await env.DB.prepare("SELECT i.id FROM holding_scan_items i JOIN holding_preparations p ON p.source_file_id=i.source_file_id WHERE p.status IN ('SCANNING','SCANNED','COMPLETE') AND (i.status='PENDING' OR (i.status='NEEDS_REVIEW' AND ((i.asset_role='DETAIL_TILE' AND i.attempts<4) OR (i.asset_role!='DETAIL_TILE' AND i.attempts<3))) OR (i.status IN ('QUEUED','RUNNING') AND i.updated_at<?)) ORDER BY CASE WHEN i.status='NEEDS_REVIEW' THEN 0 ELSE 1 END,i.entry_index LIMIT 100").bind(stale()).all()).results||[];
 const scanQueue=env.HOLDING_SCAN_QUEUE||env.DEPARTMENT_QUEUE;
 for(const row of tasks){const c=await env.DB.prepare("UPDATE holding_scan_items SET status='QUEUED',updated_at=? WHERE id=? AND (status='PENDING' OR (status='NEEDS_REVIEW' AND ((asset_role='DETAIL_TILE' AND attempts<4) OR (asset_role!='DETAIL_TILE' AND attempts<3))) OR (status IN ('QUEUED','RUNNING') AND updated_at<?))").bind(now(),row.id,stale()).run();if(!c.meta.changes)continue;try{await scanQueue.send({kind:'HOLDING_SCAN',id:row.id});}catch(e){await env.DB.prepare("UPDATE holding_scan_items SET status='PENDING' WHERE id=? AND status='QUEUED'").bind(row.id).run();throw e;}}
...
export async function processHoldingScan(body,env){
 ...
 try{
  ...
  if(nativeCapture)prompt+=nativeCapturePrompt();
