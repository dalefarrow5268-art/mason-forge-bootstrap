...
export function validScan(r){return r&&r.coverage==='COMPLETE'&&Array.isArray(r.unreadableRegions)&&r.unreadableRegions.length===0&&categories.includes(r.category)&&Array.isArray(r.findings)&&r.findings.length>0&&r.findings.every(f=>typeof f.content==='string'&&f.content.trim()&&typeof f.location==='string'&&f.location.trim());}
export async function queueHoldingScan(env){
 ...
 for(const p of waiting){
  const reader=new ZipReader(new R2Reader(env.PROJECT_FILES,p.r2_key,p.size_bytes));let index=0,count=0;
  try{for await(const e of reader.getEntriesGenerator()){
   const i=index++;if(e.directory)continue;if(!safe(e.filename)||e.encrypted||e.symlink||e.uncompressedSize>20*1024**2)throw Error('Prepared scan entry is invalid');
   await env.DB.prepare('INSERT OR IGNORE INTO holding_scan_items(id,source_file_id,entry_index,original_path,size_bytes,updated_at) VALUES(?,?,?,?,?,?)').bind(`scan-${p.source_file_id}-${p.prepared_file_id}-${i}`,p.source_file_id,i,e.filename,e.uncompressedSize,now()).run();count++;
  }}finally{await reader.close();}
  if(count!==p.units_done)throw Error('Scanner inventory differs from preparation manifest');
 ...
 for(const p of scans){const items=(await env.DB.prepare('SELECT * FROM holding_scan_items WHERE source_file_id=? ORDER BY entry_index').bind(p.source_file_id).all()).results||[];const complete=items.length===p.units_done&&items.every(x=>x.status==='COMPLETE');await env.DB.prepare('UPDATE holding_preparations SET status=?,error=?,updated_at=? WHERE source_file_id=?').bind(complete?'SCANNED':'NEEDS_REVIEW',complete?null:'Brain scanner found unreadable or incomplete coverage; review scanner records',now(),p.source_file_id).run();}
}
export async function processHoldingScan(body,env){
 ...
 try{
  ...
  const r=await askSource(env,fid,null,`Review the ENTIRE supplied page or file before project phases begin. This is a detailed source record, not a summary. Read all visible titles, sheet IDs, notes, dimensions and units, legends, schedules and tables (including rows), symbols, details, keynotes, callouts, revisions, cross references and drawing scale labels. Describe photos. Record every observed item with its location and exact text where legible. Do not infer obscured text, dimensions or measurements. A printed scale is not verified. Return JSON {category:Plans|Documents|Photos|Geotech,coverage:COMPLETE|PARTIAL,unreadableRegions:[{location,reason}],findings:[{kind,location,content}],sheetId:string,scaleVerified:false}. If content is unreadable, review cannot fit within the response, or any region was not inspected, return PARTIAL and identify each gap. Do not claim complete coverage from a summary.`,{originalPath:i.original_path,originalHoldingFileId:i.source_file_id});
  const brainKey=`${root}/reviews/${i.id}.json`;
  await env.PROJECT_FILES.put(brainKey,JSON.stringify({reviewedAt:now(),sourceFileId:fid,originalHoldingFileId:i.source_file_id,originalPath:i.original_path,preparedPackageFileId:source.id,scaleVerified:false,review:r,verification:'MODEL_REVIEW_NOT_INDEPENDENT_VERIFICATION'}));
