    ...
    for job in jobs:
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                query("UPDATE holding_preparations SET status='READY',prepared_file_id=?,manifest_key=?,source_sha256=?,prepared_sha256=?,files_total=?,pages_total=?,units_done=?,finished_at=?,updated_at=?,error=NULL WHERE source_file_id=?",[fid,mk,m['sourceSha256'],m['preparedSha256'],len(m['files']),m['pagesTotal'],m['unitsDone'],at,at,sid])
                print(json.dumps({'sourceFileId':sid,'preparation':'READY_FOR_PHASE_ONE','files':len(m['files']),'pages':m['pagesTotal'],'units':m['unitsDone']}),flush=True)
